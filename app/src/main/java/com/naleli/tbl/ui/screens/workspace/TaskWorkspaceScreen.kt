    ...
    LazyColumn(
        ...
        item {
            ...
            Row(modifier = Modifier.fillMaxWidth().padding(top = 6.dp), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                Text(task.tier.label(), style = MaterialTheme.typography.labelMedium, color = task.tier.color())
                Text("~${task.estimatedMinutes} min", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            state.assessment?.let { assessment ->
                if (assessment.result != CompetenceResult.NOT_YET_ASSESSED || assessment.submittedAt != null) {
                    Spacer(Modifier.height(6.dp))
                    Text(state.progressState.label(), style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.primary)
                }
            }
        }
